...
public class AttackHarness : MonoBehaviour 
{
	...
	public bool OccupyClosestAttackSlot(GameObject attacker, out int attackSlot, RAINNavigator navigator = null)
	{
		...
		for (int i = 0; i < openList.Count; i++)
		{
            ...
            if (navigator != null)
            {
                if (NavigationManager.Instance.GraphsForPoint(attackPosition, navigator.AI.Motor.MaxHeightOffset, NavigationManager.GraphType.Navmesh,
                ((BasicNavigator)navigator).GraphTags).Count == 0)
                    continue;
            }
